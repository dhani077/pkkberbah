...
class PokjaIICon extends BaseController
{
    ...
    public function save()
    {
        ...
        if ($btn == "tambah") {
            ...
            if (empty($jml_klp_bkb)) {
                $jml_klp_bkb = 0;
            }
            $jml_ibu_peserta_bkb = $this->request->getVar('jml_ibu_peserta_bkb');
            if (empty($jml_ibu_peserta_bkb)) {
                $jml_ibu_peserta_bkb = 0;
            }
            $jml_ape_bkb = $this->request->getVar('jml_ape_bkb');
            if (empty($jml_ape_bkb)) {
                $jml_ape_bkb = 0;
            }
            $jml_klp_simulasi_bkb = $this->request->getVar('jml_klp_simulasi_bkb');
            if (empty($jml_klp_simulasi_bkb)) {
                $jml_klp_simulasi_bkb = 0;
            }
            $jml_tuto_kf = $this->request->getVar('jml_tuto_kf');
            if (empty($jml_tuto_kf)) {
                $jml_tuto_kf = 0;
            }
            $jml_tuto_paud_kader = $this->request->getVar('jml_tuto_paud_kader');
            if (empty($jml_tuto_paud_kader)) {
                $jml_tuto_paud_kader = 0;
            }
            $jml_bkb_kader = $this->request->getVar('jml_bkb_kader');
            if (empty($jml_bkb_kader)) {
                $jml_bkb_kader = 0;
            }
            $jml_koperasi_kader = $this->request->getVar('jml_koperasi_kader');
            if (empty($jml_koperasi_kader)) {
                $jml_koperasi_kader = 0;
            }
            $jml_ktrmpilan_kader = $this->request->getVar('jml_ktrmpilan_kader');
            if (empty($jml_ktrmpilan_kader)) {
                $jml_ktrmpilan_kader = 0;
            }
            $jml_lp3_kader_latih = $this->request->getVar('jml_lp3_kader_latih');
            if (empty($jml_lp3_kader_latih)) {
                $jml_lp3_kader_latih = 0;
            }
            $jml_tpk3_kader_latih = $this->request->getVar('jml_tpk3_kader_latih');
            if (empty($jml_tpk3_kader_latih)) {
                $jml_tpk3_kader_latih = 0;
            }
            $jml_damas_kader_latih = $this->request->getVar('jml_damas_kader_latih');
            if (empty($jml_damas_kader_latih)) {
                $jml_damas_kader_latih = 0;
            }
            $jml_klp_pemula_prakop = $this->request->getVar('jml_klp_pemula_prakop');
            if (empty($jml_klp_pemula_prakop)) {
                $jml_klp_pemula_prakop = 0;
            }
            $jml_psrt_pemula_prakop = $this->request->getVar('jml_psrt_pemula_prakop');
            if (empty($jml_psrt_pemula_prakop)) {
                $jml_psrt_pemula_prakop = 0;
            }
            $jml_klp_madya_prakop = $this->request->getVar('jml_klp_madya_prakop');
            if (empty($jml_klp_madya_prakop)) {
                $jml_klp_madya_prakop = 0;
            }
            $jml_psrt_madya_prakop = $this->request->getVar('jml_psrt_madya_prakop');
            if (empty($jml_psrt_madya_prakop)) {
                $jml_psrt_madya_prakop = 0;
            }
            $jml_klp_utama_prakop = $this->request->getVar('jml_klp_utama_prakop');
            if (empty($jml_klp_utama_prakop)) {
                $jml_klp_utama_prakop = 0;
            }
            $jml_psrt_utama_prakop = $this->request->getVar('jml_psrt_utama_prakop');
            if (empty($jml_psrt_utama_prakop)) {
                $jml_psrt_utama_prakop = 0;
            }
            $jml_klp_mandiri_prakop = $this->request->getVar('jml_klp_mandiri_prakop');
            if (empty($jml_klp_mandiri_prakop)) {
                $jml_klp_mandiri_prakop = 0;
            }
            $jml_psrt_mandiri_prakop = $this->request->getVar('jml_psrt_mandiri_prakop');
            if (empty($jml_psrt_mandiri_prakop)) {
                $jml_psrt_mandiri_prakop = 0;
            }
            $jml_klp_kophkm = $this->request->getVar('jml_klp_kophkm');
            if (empty($jml_klp_kophkm)) {
                $jml_klp_kophkm = 0;
            }
            $jml_anggota_kophkm = $this->request->getVar('jml_anggota_kophkm');
            if (empty($jml_anggota_kophkm)) {
                $jml_anggota_kophkm = 0;
            }
            $data = [
                'tanggal' => $tanggal,
                'tahun' => $tahun,
                'kd_wilayah' => $this->request->getVar('kd_wilayah'),
                'jml_wrg_tiga_buta' => $jml_wrg_tiga_buta,
                'jml_klp_bljr_paketa' => $jml_klp_bljr_paketa,
                'jml_wrg_bljr_paketa' => $jml_wrg_bljr_paketa,
                'jml_klp_bljr_paketb' => $jml_klp_bljr_paketb,
                'jml_wrg_bljr_paketb' => $jml_wrg_bljr_paketb,
                'jml_klp_bljr_paketc' => $jml_klp_bljr_paketc,
                'jml_wrg_bljr_paketc' => $jml_wrg_bljr_paketc,
                'jml_klp_bljr_kf' => $jml_klp_bljr_kf,
                'jml_klp_bljr_kf_wrgbljr' => $jml_klp_bljr_kf_wrgbljr,
                'jml_klp_bljr_paud' => $jml_klp_bljr_paud,
                'jml_perpus' => $jml_perpus,
                'jml_klp_bkb' => $jml_klp_bkb,
                'jml_ibu_peserta_bkb' => $jml_ibu_peserta_bkb,
                'jml_ape_bkb' => $jml_ape_bkb,
                'jml_klp_simulasi_bkb' => $jml_klp_simulasi_bkb,
                'jml_tuto_kf' => $jml_tuto_kf,
                'jml_tuto_paud_kader' => $jml_tuto_paud_kader,
                'jml_bkb_kader' => $jml_bkb_kader,
                'jml_koperasi_kader' => $jml_koperasi_kader,
                'jml_ktrmpilan_kader' => $jml_ktrmpilan_kader,
                'jml_lp3_kader_latih' => $jml_lp3_kader_latih,
                'jml_tpk3_kader_latih' => $jml_tpk3_kader_latih,
                'jml_damas_kader_latih' => $jml_damas_kader_latih,
                'jml_klp_pemula_prakop' => $jml_klp_pemula_prakop,
                'jml_psrt_pemula_prakop' => $jml_psrt_pemula_prakop,
                'jml_klp_madya_prakop' => $jml_klp_madya_prakop,
                'jml_psrt_madya_prakop' => $jml_psrt_madya_prakop,
                'jml_klp_utama_prakop' => $jml_klp_utama_prakop,
                'jml_psrt_utama_prakop' => $jml_psrt_utama_prakop,
                'jml_klp_mandiri_prakop' => $jml_klp_mandiri_prakop,
                'jml_psrt_mandiri_prakop' => $jml_psrt_mandiri_prakop,
                'jml_klp_kophkm' => $jml_klp_kophkm,
                'jml_anggota_kophkm' => $jml_anggota_kophkm,
