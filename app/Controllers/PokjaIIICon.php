...
class PokjaIIICon extends BaseController
{
    ...
    public function save()
    {
        ...
        if ($btn == "tambah") {
            ...
            if (empty($jml_kader_pangan)) {
                $jml_kader_pangan = 0;
            }
            $jml_kader_sandang = $this->request->getVar('jml_kader_sandang');
            if (empty($jml_kader_sandang)) {
                $jml_kader_sandang = 0;
            }
            $jml_kader_tatart = $this->request->getVar('jml_kader_tatart');
            if (empty($jml_kader_tatart)) {
                $jml_kader_tatart = 0;
            }
            $jml_mknpokok_beras = $this->request->getVar('jml_mknpokok_beras');
            if (empty($jml_mknpokok_beras)) {
                $jml_mknpokok_beras = 0;
            }
            $jml_mknpokok_nonberas = $this->request->getVar('jml_mknpokok_nonberas');
            if (empty($jml_mknpokok_nonberas)) {
                $jml_mknpokok_nonberas = 0;
            }
            $jml_pmnfaatn_ternak = $this->request->getVar('jml_pmnfaatn_ternak');
            if (empty($jml_pmnfaatn_ternak)) {
                $jml_pmnfaatn_ternak = 0;
            }
            $jml_pmnfaatn_ikan = $this->request->getVar('jml_pmnfaatn_ikan');
            if (empty($jml_pmnfaatn_ikan)) {
                $jml_pmnfaatn_ikan = 0;
            }
            $jml_pmnfaatn_warung = $this->request->getVar('jml_pmnfaatn_warung');
            if (empty($jml_pmnfaatn_warung)) {
                $jml_pmnfaatn_warung = 0;
            }
            $jml_pmnfaatn_lumbung = $this->request->getVar('jml_pmnfaatn_lumbung');
            if (empty($jml_pmnfaatn_lumbung)) {
                $jml_pmnfaatn_lumbung = 0;
            }
            $jml_pmnfaatn_toga = $this->request->getVar('jml_pmnfaatn_toga');
            if (empty($jml_pmnfaatn_toga)) {
                $jml_pmnfaatn_toga = 0;
            }
            $jml_pmnfaatn_tnmkeras = $this->request->getVar('jml_pmnfaatn_tnmkeras');
            if (empty($jml_pmnfaatn_tnmkeras)) {
                $jml_pmnfaatn_tnmkeras = 0;
            }
            $jml_ind_pangan = $this->request->getVar('jml_ind_pangan');
            if (empty($jml_ind_pangan)) {
                $jml_ind_pangan = 0;
            }
            $jml_ind_sandang = $this->request->getVar('jml_ind_sandang');
            if (empty($jml_ind_sandang)) {
                $jml_ind_sandang = 0;
            }
            $jml_ind_jasa = $this->request->getVar('jml_ind_jasa');
            if (empty($jml_ind_jasa)) {
                $jml_ind_jasa = 0;
            }
            $jml_rmh_layak = $this->request->getVar('jml_rmh_layak');
            if (empty($jml_rmh_layak)) {
                $jml_rmh_layak = 0;
            }
            $jml_rmh_tidaklayak = $this->request->getVar('jml_rmh_tidaklayak');
            if (empty($jml_rmh_tidaklayak)) {
                $jml_rmh_tidaklayak = 0;
            }
            $data = [
                'tanggal' => $tanggal,
                'tahun' => $tahun,
                'kd_wilayah' => $this->request->getVar('kd_wilayah'),
                'jml_kader_pangan' => $jml_kader_pangan,
                'jml_kader_sandang' => $jml_kader_sandang,
                'jml_kader_tatart' => $jml_kader_tatart,
                'jml_mknpokok_beras' => $jml_mknpokok_beras,
                'jml_mknpokok_nonberas' => $jml_mknpokok_nonberas,
                'jml_pmnfaatn_ternak' => $jml_pmnfaatn_ternak,
                'jml_pmnfaatn_ikan' => $jml_pmnfaatn_ikan,
                'jml_pmnfaatn_warung' => $jml_pmnfaatn_warung,
                'jml_pmnfaatn_lumbung' => $jml_pmnfaatn_lumbung,
                'jml_pmnfaatn_toga' => $jml_pmnfaatn_toga,
                'jml_pmnfaatn_tnmkeras' => $jml_pmnfaatn_tnmkeras,
                'jml_ind_pangan' => $jml_ind_pangan,
                'jml_ind_sandang' => $jml_ind_sandang,
                'jml_ind_jasa' => $jml_ind_jasa,
                'jml_rmh_layak' => $jml_rmh_layak,
                'jml_rmh_tidaklayak' => $jml_rmh_tidaklayak,
