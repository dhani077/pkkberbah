...
<div class="col">
    <form action="/users/lihatkegiatanposyandu/<?= $kdposyandu; ?>" method="post" enctype="multipart/form-data">
        <?= csrf_field(); ?>
        <p><b>POSYANDU <?= $namapos; ?></b></p>
        <p><b>Data kegiatan posyandu</b></p>
                ...
                <div class="custom-file">
                    <select class="form-control <?= ($validation->hasError('range')) ? 'is-invalid' : ''; ?>" id="awal" name="awal" autofocus>
                        <?php foreach ($kegiatanA as $k) : ?>
                            <option value="<?= $k['tanggal']; ?>"><?= $k['tanggal']; ?></option>
                        <?php endforeach; ?>
                    </select>
