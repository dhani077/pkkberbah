...
<div class="col">
    <form action="/users/lihatcatatankegwarga" method="post" enctype="multipart/form-data">
        <?= csrf_field(); ?>
        <p><b>Catatan Data Dan Kegiatan Warga TP PKK Kecamatan</b></p>
            ...
            <div class="col-sm-3">
                <div class="custom-file">
                    <select class="form-control <?= ($validation->hasError('range')) ? 'is-invalid' : ''; ?>" id="awal" name="awal" autofocus>
                        <?php foreach ($catatankegwarga as $ck) : ?>
                            <option value="<?= $ck['tanggal']; ?>"><?= $ck['tanggal']; ?></option>
                        <?php endforeach; ?>
                    </select>
