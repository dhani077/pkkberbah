...
<div class="col">
    <form action="/users/lihatpelatihankader/<?= $kdwilayah; ?>" method="post" enctype="multipart/form-data">
        <?= csrf_field(); ?>
        <p><b>Kader Desa/Kelurahan <?= $kelurahan; ?></b></p>
        <div class="form-group col">
            <label for="awal" class="col-sm">Tanggal masuk sebagai Kader dari tanggal</label>
            <div class="col-sm-3">
                <div class="custom-file">
                    <select class="form-control <?= ($validation->hasError('range')) ? 'is-invalid' : ''; ?>" id="awal" name="awal" autofocus>
                        <?php foreach ($pelatihan as $p) : ?>
                            <option value="<?= $p['tgl_masuk']; ?>"><?= $p['tgl_masuk']; ?></option>
                        <?php endforeach; ?>
                    </select>
