...
<br><br><br>
<p><b>
        <center>Grafik Uang Masuk <br>Periode <?= date('d F Y', strtotime($awal)); ?> sampai <?= date('d F Y', strtotime($akhir)); ?></center>
    </b></p>
        ...
        data: {
            ...
            datasets: [
                ...
                <?php foreach ($uangmasuk as $um) : ?> {
                        label: '<?= date('d F Y', strtotime($um['tanggal'])); ?>',
                        data: ['<?= $um['jml_terima']; ?>', ],
                        backgroundColor: [
                            // 'rgba(255, 99, 132, 0.2)',
                            // 'rgba(54, 162, 235, 0.2)',
...
<br><br>
<a href="/users/rangeuangmasuk" class="btn btn-danger">Kembali</a>
<?= $this->endSection(); ?>