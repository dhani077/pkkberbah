...
<div class="col">
    <h5 class="mb-5">Form tambah jenis buku</h5>
    <form action="/TamanbacaanCon/saveBuku/<?= $kdtaman; ?>" method="post" enctype="multipart/form-data">
        <?= csrf_field(); ?>
        <input type="hidden" name="kd_taman" value="<?= $kdtaman; ?>">
        <div class="form-group row">
            <label for="jns_buku" class="col-sm-2 col-form-label">Jenis Buku</label>
            <div class="col-sm-6">
                <input type="text" class="form-control <?= ($validation->hasError('jns_buku')) ? 'is-invalid' : ''; ?>" id="jns_buku" name="jns_buku" value="<?= old('jns_buku'); ?>" autofocus>
